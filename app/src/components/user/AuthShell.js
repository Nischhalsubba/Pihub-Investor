...
const AuthShell = ({
  ...
  const accessPanelStyle = workspaceNav ? { alignItems: 'flex-start' } : undefined;
  const accessCardStyle = workspaceNav ? { marginTop: 'auto', marginBottom: 'auto' } : undefined;

  return (
    <main className={`auth-world${wide ? ' auth-world-wide-form' : ''}${hideVisual ? ' auth-world-no-visual' : ''}${workspaceNav ? ' auth-world-access' : ''}`}>
      <section className="auth-form-panel" style={accessPanelStyle}>
        <div className={`auth-card${wide ? ' auth-card-wide' : ''}`} data-motion="auth-card" style={accessCardStyle}>
