  ...
  renderRows = data => {
    ...
    return data.map((item, index) => {
      ...
      return <div className="decision-row" role="row" key={key}>
        ...
        <span role="cell" className="decision-open" data-label="Action"><Link className="row-open-link" to={detailPath} aria-label={`Open ${creditorName} ${productTitle}`}>Open</Link></span>
      </div>;
    });
  };
