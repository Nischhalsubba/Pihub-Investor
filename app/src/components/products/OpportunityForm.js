...
class OpportunityForm extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isSubmitting: false,
      uploadProgress: 0,
      statusMessage: draft ? 'Draft restored from this browser session.' : ''
    };
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.state_ids !== this.state.state_ids && this.state.state_ids.length) {
      this.props.getCounties(this.state.state_ids);
    }
  ...
  readDraft = () => {
    try {
      const value = sessionStorage.getItem(this.draftKey());
  ...
  persistDraft = () => {
    try {
      sessionStorage.setItem(this.draftKey(), JSON.stringify({ form: JSON.parse(this.draftSnapshot(this.state)), savedAt: Date.now() }));
    } catch (error) {
  ...
  updateStates = values => {
    this.setState(prev => ({
      ...
      reviewMode: false,
      statusMessage: '',
      errors: { ...prev.errors, state_ids: undefined, county_ids: undefined }
    }));
    if (!values.length) this.props.getCounties([]);
  };

  updateRating = (id, value) => this.setState(prev => ({
    ratingValues: { ...prev.ratingValues, [id]: value },
    dirty: true,
    reviewMode: false,
    statusMessage: ''
  }));
  ...
  validate = () => {
    const errors = validateOpportunity(this.state);
  ...
  buildPayload = () => {
    const ratings = CREDIT_AGENCIES.map(agency => ({
      rating_id: agency.id,
      value: String(this.state.ratingValues[agency.id] || '').trim()
    })).filter(rating => rating.value);

    return {
      product_title: this.state.product_title.trim(),
      state_ids: this.state.state_ids,
      county_ids: this.state.county_ids,
      industry_id: this.state.industry_id,
      services: { value: this.state.service_id },
      min_time_duration: Number(this.state.min_time_duration),
      max_time_duration: Number(this.state.max_time_duration),
      min_duration: Number(this.state.min_time_duration),
      max_duration: Number(this.state.max_time_duration),
      min_credit_amount: Number(this.state.min_credit_amount),
      max_credit_amount: Number(this.state.max_credit_amount),
      min_sales_creditor: Number(this.state.min_sales_creditor),
      colatoral: this.state.colatoral,
  ...
  handleSaveDraft = () => {
    this.persistDraft();
    this.setState({ statusMessage: 'Draft saved in this browser session.' });
  };
  ...
  handleSubmit = async event => {
    ...
    const result = await this.props.onCommit(this.buildPayload(), progress => {
      if (this.mounted) this.setState({ uploadProgress: progress });
    });

    if (!this.mounted) return;
    if (result === false) {
      this.setState({ isSubmitting: false, statusMessage: 'Save failed. Your draft is still available in this browser session.' });
      return;
    }

    this.clearDraft();
    this.setState({ dirty: false, isSubmitting: false, statusMessage: 'Saved.' });
  };

  renderError = field => this.state.errors[field]
    ? <span className="field-error" id={`opportunity-${field}-error`} role="alert">{this.state.errors[field]}</span>
    : null;
    ...
    return (
      <form className="profile-edit-form opportunity-form" onSubmit={this.handleSubmit} noValidate>
        ...
        <div className="form-intro-note">
          <strong>Complete underwriting inputs precisely.</strong>
          <span>Fields marked * are required. Exact financial values are entered directly instead of approximated with sliders.</span>
        </div>

        {errorCount ? (
          <div className="auth-error form-error-summary" role="alert" aria-live="assertive">
            <strong>{errorCount} field{errorCount === 1 ? '' : 's'} need attention.</strong>
            <span>Review the inline messages below. Focus has moved to the first invalid field.</span>
          </div>
        ) : null}

        <section className="profile-edit-section opportunity-form-section" aria-labelledby="opportunity-basics-title">
          <div className="profile-edit-section-head"><div><h2 id="opportunity-basics-title">Opportunity basics</h2><p>Identity, facility and geographic scope.</p></div></div>
          <div className="profile-edit-grid">
            <div className="profile-edit-field profile-edit-field-wide">
              <label htmlFor="opportunity-product_title">Opportunity title *</label>
              <input id="opportunity-product_title" className="form-control" value={this.state.product_title} onChange={event => this.update('product_title', event.target.value)} aria-invalid={Boolean(this.state.errors.product_title)} aria-describedby={this.state.errors.product_title ? 'opportunity-product_title-error' : undefined} />
              {this.renderError('product_title')}
            </div>
            <div className="profile-edit-field">
              <label htmlFor="opportunity-service_id">Facility type *</label>
              <Select inputId="opportunity-service_id" options={serviceOptions} value={serviceOptions.find(option => String(option.value) === String(this.state.service_id)) || null} onChange={option => this.update('service_id', option ? option.value : '')} placeholder="Select a facility" />
              {this.renderError('service_id')}
            </div>
            <div className="profile-edit-field">
              <label htmlFor="opportunity-state_ids">States *</label>
              <Select inputId="opportunity-state_ids" isMulti options={stateOptions} value={optionValues(stateOptions, this.state.state_ids)} onChange={options => this.updateStates(unique((options || []).map(option => option.value)))} placeholder="Select states" />
              {this.renderError('state_ids')}
            </div>
            <div className="profile-edit-field">
              <label htmlFor="opportunity-county_ids">Counties *</label>
              <Select inputId="opportunity-county_ids" isMulti options={countyOptions} value={optionValues(countyOptions, this.state.county_ids)} onChange={options => this.update('county_ids', unique((options || []).map(option => option.value)))} placeholder={this.state.state_ids.length ? 'Select counties' : 'Select states first'} isDisabled={!this.state.state_ids.length} />
              {this.renderError('county_ids')}
            </div>
            <div className="profile-edit-field">
              <label htmlFor="opportunity-industry_id">Industries *</label>
              <Select inputId="opportunity-industry_id" isMulti options={industryOptions} value={optionValues(industryOptions, this.state.industry_id)} onChange={options => this.update('industry_id', unique((options || []).map(option => option.value)))} placeholder="Select industries" />
              {this.renderError('industry_id')}
            </div>
          </div>
        </section>

        <section className="profile-edit-section opportunity-form-section" aria-labelledby="opportunity-credit-title">
          <div className="profile-edit-section-head"><div><h2 id="opportunity-credit-title">Credit parameters</h2><p>Use exact values so the opportunity can be compared and screened reliably.</p></div></div>
          <div className="profile-edit-grid">
        ...
        <section className="profile-edit-section opportunity-form-section" aria-labelledby="opportunity-risk-title">
          <div className="profile-edit-section-head"><div><h2 id="opportunity-risk-title">Risk requirements</h2><p>Collateral and external rating requirements used during screening.</p></div></div>
          <div className="profile-edit-grid">
