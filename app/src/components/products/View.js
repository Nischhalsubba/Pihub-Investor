...
class ViewProduct extends Component {
  ...
  renderRatings = ratings => {
    ...
    return ratings.filter(Boolean).map((rating, index) => {
      ...
      return (
        <div className="detail-item" key={`${name}-${index}`}>
          <span>{name}</span>
          <strong>{value}</strong>
        </div>
      );
  ...
  renderDocuments = documents => {
    ...
    return documents.filter(Boolean).map((doc, index) => (
      <button
        className="document-row"
        type="button"
        key={`${toText(doc.path) || toText(doc.file_name) || 'document'}-${index}`}
      ...
      >
        ...
        <span className="document-copy">
          <strong>{toText(doc.file_name) || `File ${index + 1}`}</strong>
          <small>{toText(doc.file_type) || 'Document'}</small>
        </span>
    ...
    return (
      <Fragment>
        ...
        <Subheader
          ...
          description={product_code ? `${isGerman ? 'Produktcode' : 'Product code'}: ${toText(product_code)}` : null}
          buttonLabel={normalizedStatus !== 'deleted' ? <Translate content="button.Produktbearbeiten" /> : null}
          link={normalizedStatus !== 'deleted' ? '/edit-product' : null}
          linkState={{ id }}
        />
        ...
        <section className="detail-hero" data-motion="metric-grid" aria-label="Product summary">
          <article>
            ...
            <strong className="detail-status-value">{this.renderStatus(normalizedStatus)}</strong>
          </article>
          <article>
          ...
          <article>
            ...
            <strong><span className="mono-value">{toText(min_time_duration) || '—'}–{toText(max_time_duration) || '—'}</span> <Translate content="label.months" /></strong>
          </article>
        </section>
...
function mapStateToProps(state) {
  return { product: state.singleProduct && state.singleProduct.product ? state.singleProduct.product : null };
}
