...
const OpportunityToolbar = props => {
  ...
  return <div className="data-toolbar" aria-label="Opportunity view tools">
    <form className="ap-query-line" onSubmit={onSearch} aria-label="Opportunity filters">
      ...
      <div className="ap-filter-tabs" role="group" aria-label="Filter by status">{statusOptions.map(option => { const active = status === option.value; return <button key={option.value || 'all'} type="button" className={active ? 'is-active' : ''} aria-pressed={active} onClick={() => setStatus(option.value)}>{option.labelKey ? <Translate content={option.labelKey} /> : t('all')}</button>; })}</div>
      <button className="ap-search-submit" type="submit">{t('search')}</button>
    </form>
    <div className="data-toolbar-actions">
      {activeFilters ? <button type="button" className="btn btn-link" onClick={clearFilters}>{t('clearFilters')} ({activeFilters})</button> : null}
      ...
      <button type="button" className="btn btn-secondary" onClick={onExport}><i className="bx bx-download" aria-hidden="true" /> {t('exportCsv')}</button>
