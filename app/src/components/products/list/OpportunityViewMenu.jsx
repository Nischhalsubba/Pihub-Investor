...
const OpportunityViewMenu = ({ density, setDensity, visibleColumns, toggleColumn, savedViews, defaultId, savedViewName, setSavedViewName, saveView, loadView, renameView, deleteView, setDefaultView, copyViewLink, exportViews, importViews }) => {
  ...
  return <details className="ap-view-menu">
    <summary><i className="bx bx-slider-alt" aria-hidden="true" />{workspaceText('view')}</summary>
    <div className="ap-view-menu-panel">
      <section className="ap-view-section">
        <header><strong>{workspaceText('density')}</strong><span>Workspace preference</span></header>
        <div className="ap-view-density" role="group" aria-label="Table density"><button type="button" aria-pressed={density === 'comfortable'} onClick={() => setDensity('comfortable')}>{workspaceText('comfortable')}</button><button type="button" aria-pressed={density === 'compact'} onClick={() => setDensity('compact')}>{workspaceText('compact')}</button></div>
      </section>
      <section className="ap-view-section">
        <header><strong>{workspaceText('columns')}</strong><span>{workspaceText('chooseFields')}</span></header>
