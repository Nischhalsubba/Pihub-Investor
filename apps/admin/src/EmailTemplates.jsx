...
export default function EmailTemplates() {
  ...
  const select = item => { setEditing({ ...item }); setSaved(false); };
  const save = event => {
    event.preventDefault();
    const next = templates.map(item => item.id === editing.id ? editing : item);
    setTemplates(next);
    writeLocal('email-templates', next);
    setSaved(true);
  };
  ...
  return <div className="ph-page-shell">
    ...
    <div className="ph-workspace-split">
      <Card title="Templates" action={<Status tone="good">{templates.length} active</Status>}>
        <div className="ph-table-wrap"><table className="ph-table"><thead><tr><th>Template</th><th>Subject</th><th>Status</th><th /></tr></thead><tbody>{templates.map(item => <tr key={item.id}><td><strong>{item.name}</strong><small>{item.id}</small></td><td>{item.subject}</td><td><Status tone="good">{item.status}</Status></td><td><button className="ph-button secondary" type="button" onClick={() => select(item)} aria-pressed={editing.id === item.id}>Edit</button></td></tr>)}</tbody></table></div>
      </Card>
      <form className="ph-card ph-stack" onSubmit={save}>
        ...
        <Field label="Template name"><input value={editing.name} onChange={event => { setSaved(false); setEditing(current => ({ ...current, name: event.target.value })); }} /></Field>
        <Field label="Subject"><input value={editing.subject} onChange={event => { setSaved(false); setEditing(current => ({ ...current, subject: event.target.value })); }} /></Field>
        <Field label="Message body"><textarea rows="8" value={editing.body} onChange={event => { setSaved(false); setEditing(current => ({ ...current, body: event.target.value })); }} /></Field>
