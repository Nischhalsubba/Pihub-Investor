...
export default function ProductMarketplace() {
  ...
  const [filters, setFilters] = useState({ creditType: '', industry: '', region: '', amount: '', term: '' });
  ...
  return <div className="ph-page-shell">
    <PageHead eyebrow="Borrower / Financing products" title="Find financing" subtitle="Search available financing structures, then start an application with the selected product carried into the borrower workflow." />
    <Card title="Search criteria" action={<button className="ph-button secondary" type="button" onClick={() => setFilters({ creditType: '', industry: '', region: '', amount: '', term: '' })}>Reset filters</button>}>
    ...
    <div className="ph-card" aria-live="polite">
      <div className="ph-card-head"><div><h2>{products.length} matching financing products</h2><p>Structured from the product-discovery flow in the PiHub reference design.</p></div></div>
      <div className="ph-table-wrap"><table className="ph-table"><thead><tr><th>Product</th><th>Credit type</th><th>Region</th><th>Amount range</th><th>Term</th><th>Requirements</th><th /></tr></thead><tbody>{products.map(product => <tr key={product.id}>
        ...
        <td><button className="ph-button primary" type="button" onClick={() => apply(product)}>Apply</button></td>
      </tr>)}</tbody></table></div>
      {!products.length ? <div className="ph-empty"><strong>No products match these criteria.</strong><span>Broaden the amount, term, industry or region filters.</span></div> : null}
    </div>
  </div>;
}
