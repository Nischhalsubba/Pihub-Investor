...
export default function ProductDetail() {
  ...
  return <div className="ph-page-shell">
    <PageHead
      eyebrow={`Borrower / Financing products / ${product.id}`}
      title={product.title}
      subtitle={product.description}
      action={alreadyApplied ? <Link className="ph-button primary" to="/financing">Continue application</Link> : <button className="ph-button primary" type="button" onClick={apply}>Apply for this product</button>}
    />
    <div className="ph-workspace-split">
      <div className="ph-stack">
        <Card title="Financing terms" action={<Status tone="good">{product.availability}</Status>}>
        ...
        <Card title="Supporting material">
          <ul className="ph-list">{product.attachments.map(name => <li key={name}><span><strong>{name}</strong><small>Reference document</small></span><Status>Available</Status></li>)}</ul>
        </Card>
      </div>
      <aside className="ph-sticky-rail">
        ...
        <Card title="Application state"><p className="ph-section-note">{alreadyApplied ? 'This product is already attached to the current browser-local application.' : 'Applying creates a borrower-owned draft. The product is not submitted to Advisory or Investor until the financing request is explicitly submitted.'}</p><div className="ph-form-actions" style={{ marginTop: 12 }}>{alreadyApplied ? <Link className="ph-button secondary" to="/applications">View applications</Link> : <button className="ph-button primary" type="button" onClick={apply}>Start application</button>}</div></Card>
      </aside>
    </div>
  </div>;
}
